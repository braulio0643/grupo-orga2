...
size_t wav_read(WavFile *self, void *buffer, size_t count, WavChannelFormat chanFmt)
{
    size_t read_count;
    ...
    int16_t *interleaved_data = (int16_t *)malloc(n_channels * count * sizeof(int16_t));

    read_count = fread(interleaved_data, sample_size, n_channels * count, self->fp);
    if (ferror(self->fp))
    {
    ...
    int valid_len = (int)read_count / n_channels;

    switch (chanFmt)
    {
    case INTERLEAVED:
    { // channel buffer is interleaved e.g. [LRLRLRLR]
        for (int pos = 0; pos < n_channels * valid_len; pos++)
        {
            ((int16_t *)buffer)[pos] = interleaved_data[pos];
        }
        break;
    }
    case INLINE:
    { // channel buffer is inlined e.g. [LLLLRRRR]
        for (int i = 0, pos = 0; i < n_channels; i++)
        {
            for (int j = i; j < valid_len * n_channels; j += n_channels, ++pos)
            {
                ((int16_t *)buffer)[pos] = interleaved_data[j];
            }
        }
        break;
    }
    ...
    { // channel buffer is split e.g. [[LLLL],[RRRR]]
        for (int i = 0; i < n_channels; i++)
        {
            ...
            {
                ((int16_t (*)[count]) buffer)[i][j] = interleaved_data[j * n_channels + i];
            }
        }
        break;
    }
...
size_t wav_write(WavFile *self, WAV_CONST void *buffer, size_t count, WavChannelFormat chanFmt)
{
    size_t write_count;
    WavU16 n_channels = wav_get_num_channels(self);
    size_t sample_size = wav_get_sample_size(self);
    ...
    int16_t *z = (int16_t *) malloc(n_channels * count * sizeof(int16_t));
    switch (chanFmt)
    {
    case INTERLEAVED:
    {
        const int16_t *const x = (const int16_t *const)buffer;
        for (unsigned i = 0; i < n_channels * count; ++i)
        {
            z[i] = x[i];
        }
        break;
    ...
    case SPLIT:
    {
        const int16_t (*x)[count] = (const int16_t (*)[count]) buffer;
        for (size_t i = 0, k = 0; i < count; ++i)
        {
            for (unsigned j = 0; j < n_channels; ++j)
            {
    ...
    write_count = fwrite(z, sample_size, n_channels * count, self->fp);
    free(z);
    if (ferror(self->fp))
    {
